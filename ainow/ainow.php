...
class AI_NOW {
       ...
       function add_front_JS() {
               ...
               wp_localize_script(
                       ...
                       array(
                               ...
                               'nonce'    => wp_create_nonce( 'ainow_nonce' ),
                       )
               );
       }
    ...
    // Register Admin CSS
       ...
       private function validate_ajax_request() {
               if ( ! check_ajax_referer( 'ainow_nonce', 'nonce', false ) ) {
                       wp_send_json_error( array( 'message' => 'Invalid nonce' ), 403 );
               }

               if ( ! $this->is_same_host() ) {
       ...
       private function is_same_host() {
               if ( empty( $_SERVER['HTTP_REFERER'] ) ) {
                       return false;
               }

               $referer = wp_parse_url( wp_unslash( $_SERVER['HTTP_REFERER'] ), PHP_URL_HOST );
               $host    = wp_unslash( $_SERVER['HTTP_HOST'] );

               return $referer && $host && $referer === $host;
       }
    ...
    function ainow_tracking() {
                ...
                $wpdb->update(
                        ...
                        array(
                               ...
                               $wpdb->insert(
                                               $ainow_table,
                                               array(
                                                                'user_uid'          => sanitize_text_field( $unique_user_id ),
                                                                'user_interests'   => sanitize_text_field( $_USER_ONPAGETITLE ),
       ...
       function ainow_setup_uuid_global() {
               ...
               if ( empty( $_SESSION['AINOW_UUID'] ) || ! isset( $_SESSION['AINOW_UUID'] ) ) {
                       $unique_user_id           = isset( $_POST['data'] ) ? sanitize_text_field( wp_unslash( $_POST['data'] ) ) : '';
    ...
    // Build UUID function handler --> Used to generate the new users UUID
       function ainow_uuid_maker() {
               ...
               $last_id = $wpdb->get_results( $sql_, OBJECT );
               $last_id = ! empty( $last_id ) ? (int) $last_id[0]->id : 1;

               $host           = sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) );
               $unique_user_id = $last_id . '@' . $host;

               $_SESSION['AINOW_UUID'] = $unique_user_id;

               $this->ainow_tracking();

               wp_send_json_success( array( 'uuid' => $unique_user_id ) );
       }
    ...
    // AINOW List shortcode implementation function
       function register_ainow_list( $atts ) {
               ...
               $load_btn_text = sanitize_text_field( $atts['load_btn_text'] );
        ...
        if ( !empty( $posts_stack ) ) { $this->add_ids_to_ignore( $posts_stack ); }
               ...
               foreach ( $posts_stack as $post_ ) {
                       ...
                       if ( get_post_status( $post_id ) !== false ) {
                               ...
                               $post_content = esc_html( wp_trim_words( get_post_field( 'post_content', $post_id ), 25, '...' ) );
                               $post_image   = esc_url( wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ), 'single-post-thumbnail' )[0] );

                               $_HTML_RESULT .= sprintf(
                                       '<a href="%1$s" class="post-anchor">
                                       <div id="post-%2$d" class="post-container">
                                               <div class="featured-image" style="background-image: url(%3$s);"></div>
                                               <div id="post-content">
                                                       <h1 class="post-title">%4$s</h1>
                                                       <div class="post-content">%5$s</div>
                                               </div>
                                       </div>
                               </a>',
                                       $post_url,
               ...
               $_HTML_RESULT .= '</div><button id="ainow-load-more-posts">' . esc_html( $load_btn_text ) . '</button>';
               return $_HTML_RESULT;
    }
    ...
    // Load More Posts function
       function ainow_load_more_posts() {
               $this->validate_ajax_request();
               global $wpdb;
               $unique_user_id = isset( $_SESSION['AINOW_UUID'] ) ? sanitize_text_field( $_SESSION['AINOW_UUID'] ) : '';
        ...
        if ( !empty( $posts_stack ) ) { $this->add_ids_to_ignore( $posts_stack ); }
               ...
               wp_send_json_success( array( 'html' => $_HTML_RESULT ) );
       }
};
